...
**How are completed shifts recorded?**

The Log shift form requires an employer and explicit start/end dates and times. The server validates the duration and break, checks for overlaps inside a locked transaction, and stores a completed shift. Editing updates the existing row. Legacy clock endpoints remain for compatibility, but the UI has no live clock.
...
The generator is capped at January 1–September 17, 2026 and uses only the specified regular schedule, excludes unspecified Apple Tuesdays and earlier Apple history, previews before insertion, and initially selects no dates. Users confirm actual dates. Imported rows retain their schedule-estimate label and durable keys, so reruns do not duplicate them.

**How do you calculate savings?**

Projected savings equals gross earned minus planned monthly expenses. Actual cash remaining equals recorded paycheck amounts minus actual expenses. Actual savings counts deposits minus withdrawals, including explicitly recorded balance adjustments. The goal shows projected progress and actual deposit progress separately. Missing paycheck data stays unknown instead of being displayed as zero.
...
**How do you distinguish earnings from paychecks?**

Gross earnings come from shift calculations and belong to the shift start date. Actual paychecks are exact manually entered amounts and belong to payday. The shared September 18 anchor only establishes biweekly payment dates; work cutoffs remain unknown until configured. Unknown paycheck amounts are null, while a manually entered zero is a real zero.

**Why separate planned expenses from transactions?**

A recurring budget is a plan, not proof of a purchase. Date-effective budget rows allow monthly projections without creating fake transactions. Actual expenses and savings deposits remain separately entered records.

**How do you calculate estimated paychecks?**

Only when the covered work dates are explicitly recorded or an employer's coverage rule is configured. Then the backend sums logged shift gross within those dates. It never assumes gross equals the actual deposit or labels the difference as tax.
